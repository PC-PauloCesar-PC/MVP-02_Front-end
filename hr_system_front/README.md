...
2.  **Navegue até a pasta do Front-end** (`hr_system_front/` - onde está o `docker-compose.yml`).
    ```bash
    cd ../hr_system_front
    ```
3.  **Execute a Aplicação:**
    ...
    Caso queira executar somente o Front-end, através de seu Dockerfile, execute os seguintes comandos no terminal dentro da pasta `hr_system_front`:

    Primeira vez, e quando fizer alterações:
    ```bash
    docker build -t hr_system_front .
    docker run -p 3000:3000 hr_system_front
    ```
    Demais vezes:
    ```bash
    docker run -p 3000:3000 hr_system_front
